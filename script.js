...
// CUSTOM FULLSCREEN FUNCTION (Safari & Mobile Compatible)
...
async function fetchSummary() {
    try {
        ...
        let recent = [...(presstv||[]), ...(meObs||[]), ...(rnintel||[])]
            ...
            .filter(p => determineEventType(p.text) !== null) 
...
async function fetchAirspaceStatus() {
    ...
    try {
        const [flightEmerg, osint, intelsky] = await Promise.all([
            scrapeTelegramChannel('FlightEmergency', false), 
            scrapeTelegramChannel('osintdefender', false),
            scrapeTelegramChannel('Intel_sky', false)
        ]);
        ...
        airspaceDB.forEach(ap => {
            ...
            if(mention) {
                ...
                if(t.match(/close|suspend|halt|shut/)) {
                    ap.status = "CLOSED"; ap.detail = "AIRSPACE CLOSED VIA OSINT";
                } else if(t.match(/reopen|resume|clear/)) {
                    ap.status = "OPEN"; ap.detail = "OPERATIONS RESUMING VIA OSINT";
                }
            }
        });
    ...
    try {
        ...
        if (response.ok) {
            ...
            externalData.forEach(ext => {
                if (typeof ext.timestamp === 'string') ext.timestamp = new Date(ext.timestamp).getTime();
                if (!ext.timestamp) ext.timestamp = Date.now();

                const exists = localData.some(loc => loc.id === ext.id || (loc.location === ext.location && Math.abs(loc.timestamp - ext.timestamp) < 60000));
                if(!exists) localData.push(ext);
            });
            
            saveStoredIntel(localData);
            globalIntelData = [...baselineData, ...localData];
            renderMapData(); 
        }
    } catch(e) {
        console.warn("No local history.json found or failed to load.");
    }
}
            ...
            if (detectedLoc) {
                ...
                if(!isDuplicate && (Date.now() - post.date.getTime()) < 3600000) {
                    storedIntel.push({
                        ...
                        source: post.channel.toUpperCase(), mediaHTML: post.mediaHTML, videoSrc: post.videoSrc 
                    });
                    newFound = true;
...
function flyToLoc(lng, lat, strikeId) {
    ...
    // Force SIREN to blink on click
    document.querySelectorAll('.siren-radius').forEach(el => el.classList.remove('siren-active'));
    const clickedSiren = document.getElementById('siren-rad-' + strikeId);
    if (clickedSiren) clickedSiren.classList.add('siren-active');

    const markerObj = activeMapMarkers.find(m => m.strikeId === strikeId);
    if (markerObj) {
        if (activePopup) activePopup.remove(); 
        markerObj.marker.togglePopup();
        activePopup = markerObj.marker.getPopup();
    }
}
...
function renderMapData() {
    activeMapMarkers.forEach(m => m.marker.remove());
    activeMapMarkers = [];
    ...
    filtered.forEach(strike => {
        ...
        if (strike.eventType === 'siren') {
            ...
            let isAutoBlinking = (minutesAgo >= 30 && minutesAgo <= 60) ? 'siren-active' : '';
            sirenCircle.className = `siren-radius ${isAutoBlinking}`;
            sirenCircle.id = 'siren-rad-' + strike.id;
        ...
        activeMapMarkers.push({ strikeId: strike.id, marker: marker });
    });
}
